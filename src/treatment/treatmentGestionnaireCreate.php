...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($utilisateur_id) {
        try {
            ...
            if (!$utilisateur) {
                ...
                $result = $utilisateurRepo->update($utilisateur_id, ['role' => 'Gestionnaire']);
                
                if ($result) {
                    $_SESSION['success'] = "L'utilisateur a été promu avec succès en tant que gestionnaire.";
        ...
        // Si aucun utilisateur n'est sélectionné, on crée un nouveau gestionnaire
    ...
    if ($mdp !== $confirmation_mdp) {
        ...
        if (empty($errors)) {
            try {
                ...
                if ($utilisateurRepo->getUserByEmail($email)) {
                    ...
                    $gestionnaire = new ModeleGestionnaire([
                        'prenom' => $prenom,
                        'nom' => $nom,
                        'email' => $email,
                        'mdp' => $mdp,
                    ...
                    $result = $utilisateurRepo->inscription($gestionnaire);
                    ...
                    if ($result) {
                        ...
                        $errors[] = "Une erreur est survenue lors de la création du gestionnaire.";
                    }
                }
            } catch (Exception $e) {
                $errors[] = "Erreur technique : " . $e->getMessage();
        ...
        // Stocker les erreurs et les données du formulaire dans la session pour les afficher dans le formulaire
        ...
        header('Location: ../../view/crudGestionnaire/gestionnaireCreate.php');
        exit();
    }
} else {
    // Si le formulaire n'a pas été soumis, rediriger vers la page de création
    header('Location: ../../view/crudGestionnaire/gestionnaireCreate.php');
    exit();
}
